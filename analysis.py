...
powerFiles = [os.path.join(path, name)
              for path, dirs, files in os.walk(targetPath)
              for name in files if name.endswith(("-power.csv"))]
...
def powerAndEntropy(powerFile, entropyFile):
    ...
    print(power.shape, entropy.shape)
...
names = ['Ajinkya','Ari','Arrow','Arun','Bhargav','Cruise','DadPatil','DC','Dipack','Doctor','Goswami','Harsh','Harvey','JE','JK','JKSimmons','Kapish','KL','Leonardo','Liam','Loki','Louis','Malkan','Manas','Mathur','MichaelCera','Nicholson','Nishant','OldGuy','Puneet','RandomMan','Rishi','Robert','Rohit','Sad','Tobymac','Tushar','Tyrion','Vaas','Akansha','Arzoo','Emily','Isha','Lorelai','MomPatil','Olivia','Pallavi','Paris','PDB','Pooja','Pritha','Sayalee','Tanvi','Unsorted','YoungWoman']
...
def main():
    ...
    for person in names:
        ...
        for emotion in emotions:
            ...
            if(result1 == 0): result1 = variance
            else: result2 = variance 

            print("Variance of", person, "for", emotion, "is", str(variance))
