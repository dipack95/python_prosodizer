...
def main():
    ...
    #inputFile = ["sounds/Women/normal_woman.wav", "sounds/Women/angry_woman.wav"]
    ...
    for tempInFile in inputFile:    
        ...
        for i in range(0, len(mfccs)):
            rms_energy.append(rms(mfccs[i,:]))

        print(min(rms_energy), max(rms_energy), tempInFile)
        
        plt.plot(rms_energy, label = tempInFile)
